"""Диалог удалённой записи видео: стриминг через FFmpeg/GStreamer и запись с удалённого хоста."""
    ...
    def _start_remote_recording(self, filepath: str, fmt: str) -> None:
        ...
        if self._audio_group.isChecked():
            if audio_source == "pulse":
                ffmpeg_args += ["-f", "pulse", "-ac", "2", "-ar", "48000", "-i", audio_input]
            else:
                alsa_input = audio_input
                if alsa_input.startswith("hw:"):
                    alsa_input = "plug" + alsa_input
                ffmpeg_args += ["-f", "alsa", "-ar", "48000", "-i", alsa_input]
