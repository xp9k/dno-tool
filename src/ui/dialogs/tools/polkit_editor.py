"""Диалог просмотра и редактирования политик Polkit (PolicyKit) на удалённом хосте через SSH."""
...
from PySide6.QtWidgets import (
    QApplication, QDialog, QVBoxLayout, QHBoxLayout, QSplitter, QTreeView,
    QLabel, QPushButton, QMessageBox, QComboBox, QLineEdit,
    QHeaderView, QStatusBar, QWidget, QFormLayout, QGroupBox,
    QProgressBar, QScrollArea, QDialogButtonBox, QTextEdit, QFrame
)
...
class PolkitEditorDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        self.right_splitter.addWidget(self.detail_scroll)

        # SED Section
        sed_container = QWidget()
        sed_layout = QVBoxLayout(sed_container)
        sed_layout.setContentsMargins(0, 10, 0, 0)
        sed_layout.setSpacing(4)

        sed_label = QLabel("Команда sed для применения изменений на удалённом хосте:")
        sed_layout.addWidget(sed_label)

        sed_row = QHBoxLayout()
        self.sed_edit = QTextEdit()
        self.sed_edit.setReadOnly(True)
        self.sed_edit.setPlaceholderText("Выберите политику и измените значения прав доступа для формирования команды sed")
        sed_row.addWidget(self.sed_edit, 1)

        self.sed_copy_btn = QPushButton("📋")
        self.sed_copy_btn.setToolTip("Копировать команду в буфер обмена")
        self.sed_copy_btn.setFixedWidth(36)
        self.sed_copy_btn.clicked.connect(self._copy_sed_command)
        sed_row.addWidget(self.sed_copy_btn)
        sed_layout.addLayout(sed_row)

        self.right_splitter.addWidget(sed_container)
        self.right_splitter.setStretchFactor(0, 1)
        self.right_splitter.setStretchFactor(1, 0)
        
        # Initial sizing for sed field to be proportional to detail area
        self.right_splitter.setSizes([500, 120])
    ...
    def _show_policy_details(self, action_id: str):
        ...
        for key, label in PERM_KEYS:
            ...
            combo.currentIndexChanged.connect(self._update_sed_command)
        ...
        self._update_sed_command()

    def _generate_sed_command(self) -> str:
        if not self._current_action_id or self._current_action_id not in self.policies_data:
            return ""
        data = self.policies_data[self._current_action_id]
        source_file = data.get('_source_file', '')
        if not source_file:
            return ""
        parts = []
        for key, _ in PERM_KEYS:
            current = self.combo_boxes[key].currentData() if key in self.combo_boxes else ''
            if current:
                parts.append(f"s|<{key}>[^<]*</{key}>|<{key}>{current}</{key}>|g")
        if not parts:
            return ""
        sed_expr = '; '.join(parts)
        return f"sudo sed -i '{sed_expr}' '{source_file}'"

    def _update_sed_command(self):
        cmd = self._generate_sed_command()
        self.sed_edit.setPlainText(cmd)

    def _copy_sed_command(self):
        cmd = self._generate_sed_command()
        if cmd:
            QApplication.clipboard().setText(cmd)
            self.sed_copy_btn.setText("✓")
            QTimer.singleShot(1200, lambda: self.sed_copy_btn.setText("📋"))
