...
from PySide6.QtWidgets import (QDialog, QHeaderView, QLabel, QLineEdit, QWidget,
                             QPushButton, QTableWidget, QVBoxLayout, QAbstractItemView,
...
from PySide6.QtCore import Qt, QObject, QThread, Signal, QMimeData
...
class ScannerTreeItem(QStandardItem):
    ...
    def update_online_status(self, is_online: bool):
        ...
        if is_online:
            ...
            # Устанавливаем статус в 4-ю колонку
            if self.index().model() and self.index().siblingAtColumn(3):
                status_item = self.index().siblingAtColumn(3).data(Qt.ItemDataRole.UserRole)
                if status_item is not None:
                    self.index().model().setData(self.index().siblingAtColumn(3), "Онлайн", Qt.ItemDataRole.DisplayRole)
                else:
                    status_item = QStandardItem("Онлайн")
                    self.setChild(self.row(), 3, status_item)
        else:
            self.setIcon(QIcon(ICONS['offline']))
...
class IPScannerTree(QTreeView):
    ...
    def __init__(self, parent=None):
        ...
        # 4 колонки: Хосты | Порты | MAC адрес | Статус
        self._model.setHorizontalHeaderLabels(["Хосты", "Порты", "MAC адрес", "Статус"])
        ...
        self.setColumnWidth(0, 250)  # Хосты
        self.setColumnWidth(1, 100)  # Порты
        self.setColumnWidth(2, 140)  # MAC адрес
        self.setColumnWidth(3, 80)   # Статус
    ...
    def _set_children_checkstate(self, parent_item, check_state):
        ...
        for i in range(parent_item.rowCount()):
            ...
            if hasattr(child, 'is_folder') and child.is_folder:
    ...
    def add_device_to_folder(self, folder: ScannerTreeItem, device: DeviceModel,
        ...
        folder.appendRow([device_item, ports_item, mac_item, status_item])
    ...
    def update_device_in_folder(self, folder: ScannerTreeItem, device: DeviceModel,
        ...
        for i in range(folder.rowCount()):
            ...
            if hasattr(child, 'device') and child.device and child.device.host == device.host:
                ...
                # Обновляем порты (колонка 1)
                if ports is not None:
                    ports_text = ", ".join(map(str, ports)) if ports else ""
                    if ports == ['icmp']:
                        ports_text = "ICMP"
                    ports_item = folder.child(i, 1)
                    if ports_item:
                        ports_item.setText(ports_text)
                # Обновляем MAC (колонка 2)
                if mac_address:
                    mac_item = folder.child(i, 2)
                    if mac_item:
                        mac_item.setText(mac_address)
                # Обновляем статус (колонка 3)
                status_item = folder.child(i, 3)
                if status_item:
...
class ScannerResultTable(QTableWidget):
    """Таблица результатов сканирования с колонками: Чекбокс, IP, Имя, Порты, MAC"""

    def __init__(self, parent=None):
        super().__init__(parent)
        self._devices = {}  # host -> DeviceModel
        self._init_table()

    def _init_table(self):
        """Инициализация таблицы"""
        self.setColumnCount(5)
        self.setHorizontalHeaderLabels(["☐", "IP адрес", "Имя хоста", "Порты", "MAC адрес"])

        header = self.horizontalHeader()
        header.setSectionResizeMode(0, QHeaderView.ResizeMode.ResizeToContents)  # Чекбокс
        header.setSectionResizeMode(1, QHeaderView.ResizeMode.Interactive)  # IP
        header.setSectionResizeMode(2, QHeaderView.ResizeMode.Stretch)  # Имя
        header.setSectionResizeMode(3, QHeaderView.ResizeMode.Interactive)  # Порты
        header.setSectionResizeMode(4, QHeaderView.ResizeMode.Interactive)  # MAC
        ...
        self.setColumnWidth(1, 140)  # IP
        self.setColumnWidth(3, 120)  # Порты
        self.setColumnWidth(4, 160)  # MAC
    ...
    def add_device(self, device: DeviceModel, is_online: bool, ports: List = None):
        """
        Добавить или обновить устройство в таблице.
        ...
        existing_row = self._find_device_row(host)

        if existing_row >= 0:
            # Обновляем существующую запись
            self._update_device_row(existing_row, device, is_online, ports)
        else:
            # Добавляем новую запись
            self._add_device_row(device, is_online, ports)

    def _find_device_row(self, host: str) -> int:
        """Найти строку устройства по IP"""
    ...
    def _add_device_row(self, device: DeviceModel, is_online: bool, ports: List = None):
        """Добавить новую строку устройства"""
        row = self.rowCount()
        self.insertRow(row)
        ...
        # IP адрес
        ip_item = QTableWidgetItem(device.host)
        ip_item.setData(Qt.ItemDataRole.UserRole, device.host)
        ...
        self.setItem(row, 3, ports_item)

        # MAC адрес
        mac_text = device.mac_address if device.mac_address else "—"
        ...
        self._update_row_visual(row, is_online)

    def _update_device_row(self, row: int, device: DeviceModel, is_online: bool, ports: List = None):
        """Обновить существующую строку устройства"""
        # Обновляем имя
        ...
        if device.mac_address:
            ...
            if mac_item:
                mac_item.setText(device.mac_address)

        # Обновляем устройство в словаре
        self._devices[device.host] = device

        # Обновляем визуальный статус
        self._update_row_visual(row, is_online)

    def _update_row_visual(self, row: int, is_online: bool):
        """Обновить визуальное отображение строки"""
        icon = QIcon(ICONS['online'] if is_online else ICONS['offline'])
        ip_item = self.item(row, 1)
        if ip_item:
            ip_item.setIcon(icon)
            font = ip_item.font()
            font.setBold(is_online)
            ip_item.setFont(font)

    def get_checked_devices(self) -> List[DeviceModel]:
        """Получить все отмеченные устройства"""
        devices = []
        for row in range(self.rowCount()):
