...
from PySide6.QtGui import (
    QSyntaxHighlighter, QTextCharFormat, QColor, QFont, QTextDocument, QAction
)
    ...
    def __init__(self, parent=None, custom_params=None):
        ...
        if custom_params:
            for param in custom_params:
                self.params_table.insertRow(row)
                param_item = QTableWidgetItem(f"%{param}%")
                param_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                self.params_table.setItem(row, 0, param_item)
                self.params_table.setItem(row, 1, QTableWidgetItem("Пользовательский параметр"))
                row += 1
