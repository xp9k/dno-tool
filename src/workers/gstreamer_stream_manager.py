...
class GStreamerStreamManager(QObject):
    ...
    def _build_rtsp_pipeline_and_cmd(self) -> str:
        ...
        if enable_audio:
            ...
            audio_codec = self._settings.get("audio_codec", "aac") or "aac"
            if audio_source != "pulse" and audio_input.startswith("hw:"):
                audio_input = "plug" + audio_input
            config["audio"] = True
    ...
    def _build_remote_cmd(self) -> str:
        ...
        if enable_audio:
            ...
            if audio_source == "pulse":
                ...
                alsa_input = audio_input
                if alsa_input.startswith("hw:"):
                    alsa_input = "plug" + alsa_input
                pipeline_parts.append(f"alsasrc device={shlex.quote(alsa_input)}")
                pipeline_parts.append("audio/x-raw,rate=48000")
