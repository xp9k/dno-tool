"""
HostPingTimer - индивидуальный таймер пинга для каждого хоста.

Каждый отмеченный хост получает собственный QTimer, который периодически
опрашивает его независимо от других хостов. Пинг выполняется через
централизованный WorkerPoolManager с ограничением конкурентности.

Архитектура: HostPingTimerManager владеет всеми таймерами и эмитит
единственный сигнал online_updated(DeviceModel, bool), на который
подписываются представления (дерево, список).
"""
