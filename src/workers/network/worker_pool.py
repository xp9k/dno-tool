"""Централизованный менеджер пулов потоков WorkerPoolManager.

Предоставляет единый ThreadPoolExecutor на всё приложение с ограничением
максимального числа одновременных задач, трекингом активных задач,
пакетной отправкой (submit_batch), сигналами Qt для мониторинга состояния
и интеграцией с WorkerBridge. Раздельные глобальные пулы для пингов и команд."""
