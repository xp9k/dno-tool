...
RESULT_SUCCESS: int = 0   # Ключ успешно установлен
RESULT_ERROR: int = 1    # Ошибка при установке
RESULT_IGNORE: int = 2  # Ключ уже присутствует
RESULT_ABORT: int = 3   # Операция прервана
...
class KeyInstallerWorker(BaseWorker):
    ...
    def __init__(
            ...
            parent=None) -> None:
        """
        Инициализация worker установки SSH-ключей.

        Args:
            devices: Список устройств для установки ключа.
            key_path: Путь к публичному SSH-ключу.
    ...
    def install_key(self, device: DeviceModel, key_path: str = None) -> tuple:
        """
        Установить SSH-ключ на устройство.

        Args:
            device: Целевое устройство.
            key_path: Путь к публичному ключу.

        Returns:
            Кортеж (host, result_code, output).
        """
        if self._local_abort.is_set():
    ...
    def _install_key_paramiko(self, device: DeviceModel, key_path: str = None) -> tuple:
        """
        Установить SSH-ключ через Paramiko (SFTP).
    ...
    def abort(self) -> None:
        """Запросить прерывание установки ключей."""
